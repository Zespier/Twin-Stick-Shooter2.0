...
public class CraftableWeaponUI : SelectableItemForController {
    ...
    public override void Use() {
        ...
        for (int i = 0; i < mineralsNeeded.Count; i++) {
            for (int m = 0; m < Ship.instanceOfClient.minerals.Length; m++) {

                MineralDrop mineralNeeded = mineralsNeeded[i];
                if (m == (int)mineralNeeded.tier && Ship.instanceOfClient.minerals[m] < mineralNeeded.amount) {
            ...
            for (int i = 0; i < mineralsNeeded.Count; i++) {
                for (int m = 0; m < Ship.instanceOfClient.minerals.Length; m++) {
                    ...
                    if (m == (int)mineralNeeded.tier) {
                        Ship.instanceOfClient.minerals[m] -= mineralNeeded.amount;
                    }
                }
            }

            Ship.instanceOfClient.AddWeapon(weaponReward);
        }
    }
}
